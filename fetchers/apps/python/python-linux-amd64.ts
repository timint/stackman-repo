...
export async function getReleases(): Promise<Release[]> {
	...
	while ((match = regex.exec(html))) {
		...
		const tgzUrl = `https://www.python.org/ftp/python/${version}/Python-${version}.tgz`;
		const tgzRes = await fetch(tgzUrl, { method: 'HEAD' });

		if (tgzRes.ok && (!releases[era] || version.localeCompare(releases[era].version, undefined, { numeric: true }) > 0)) {
			releases[era] = {
				id: `python-${era}`,
				name: 'Python',
				version,
